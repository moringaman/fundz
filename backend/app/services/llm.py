...
class LLMService:
    ...
    async def generate_signal(self, indicators: Dict[str, Any], price_data: Dict[str, Any]) -> LLMResponse:
...
3. CONFLUENCE: Look for overlapping signals (e.g., RSI oversold + price at lower BB band = buy confluence)
...
DECISION RULES:
- BUY: RSI <40 + MACD bullish crossover + price at/below middle band + uptrend structure
- SELL: RSI >60 + MACD bearish crossover + price at/above middle band + downtrend structure
- HOLD: Conflicting signals, uncertain trend, or narrow range consolidation

CONFIDENCE CALIBRATION:
- High (0.7-1.0): 3+ confluent signals, clear trend, MACD + RSI aligned
- Medium (0.4-0.7): 2 aligned signals, mixed momentum, unclear trend
- Low (0.0-0.4): Conflicting signals, choppy action, indecision

Return JSON: {{"action": "buy|sell|hold", "confidence": 0.0-1.0, "reasoning": "brief but specific technical analysis reasoning", "key_levels": {{"resistance": 0.0, "support": 0.0}}, "risk_level": "low|medium|high"}}
"""
